...
要使用 `torch.optim` 包，你必须先构造一个优化器对象，其保存了当前状态并根据计算的梯度更新模型参数。

#### 构造优化器

为了构造一个 `Optimizer` 对象，你需要传入一个包含要优化的模型参数（应为 `Variable` 实例）的可迭代对象，然后指定优化器相关的选项，例如学习率、权重衰减等等。
...
!!! note "注意"
    ...
    如果你需要将一个模型移动到 GPU 中（通过调用 `.cuda()` 或 `.to()`），请在为此模型构造优化器之前完成这一操作。`.cuda()` 或 `.to()` 调用之后的模型的参数将会是另一组不同的对象。

    总的来说，你需要保证在优化器构建和使用的整个过程中，被优化的模型参数存在于一致的位置。

#### 分组指定选项

`Optimizer` 也支持为多组参数分别指定选项，这时需要传入一个包含多个字典的可迭代对象，其中每个字典定义一个单独的参数组。字典应包含一个 `params` 键，对应一组参数；其余的键应匹配 `Optimizer` 实例接受的关键字参数，并用作这一组参数的选项。
...
optim.SGD([{'params': model.base.parameters()},
           {'params': model.classifier.parameters(), 'lr': 1e-3}],  # 为这一组参数重载学习率
           lr=1e-2, momentum=0.9)                              # 关键字参数作为全局的默认选项
```
...
所有的优化器都实现了 `step()` 方法，其基于计算的梯度值更新参数。可以有两种使用方法：
...
`torch.optim.lr_scheduler` 提供了数种基于回合数调整学习率的规划器。`torch.optim.lr_scheduler.ReduceLROnPlateau` 允许基于一些验证指标来动态地降低学习率。
