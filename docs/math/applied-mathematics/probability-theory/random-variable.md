...
**例 2.1** 下图所示的二维离散型随机向量 $X=(X_1,X_2)$ 的概率分布为
...
**例 2.7** 设 $X=(X_1,\cdots,X_n)$ 服从多项分布 $M(N;p_1,\cdots,p_n)$，要求其边缘分布。例如，考虑分量 $X_1$，我们把事件 $A_1$ 作为一方，事件 $A_2+\cdots+A_n$ 作为另一方（也就是 $\overline{A}_1$），那么 $X_1$ 就是在 $N$ 次独立试验中事件 $A_1$ 发生的次数，而在每次试验中 $A_1$ 发生的概率都是 $p_1$。经过这一分析，无需计算就可以知道：$X_1$ 的分布就是二项分布 $B(N,p_1)$。应用 (2.8) 也可以得到这个结果：按照 (2.8)，注意到多项分布的形式 (2.3)，有
...
**例 4.5** 若 $X\sim N(0,1)$，求 $Y=X^2$ 的密度函数。
...
按照 (4.11)，得到 $(Y_1,Y_2)$ 的密度函数为

$$
l(y_1,y_2)=f(b_{11}y_1+b_{12}y_2,b_{21}y_1+b_{22}y_2)|b_{11}b_{22}-b_{12}b_{21}|
$$

**例 4.6** 再次考虑例 3.6。为与此处记号一致，把该例中的 $R,\Theta$ 分别记为 $Y_1,Y_2$。这时逆变换 (4.8) 为
...
$$
f(x_1,x_2)=\frac{1}{2\pi}\exp(-\frac{1}{2}(x_1^2+x_2^2))
$$

而 $x_1^2+x_2^2=y_1^2\cos^2y_2+y_1^2\sin^2y_2=y_1^2$，由 (4.11)，得到 $(Y_1,Y_2)$ 的概率密度函数为 $\frac{1}{2\pi}e^{-y_1^2/2}y_1$，变量取值范围为 $0\le y_1<\infty,0\le y_2<2\pi$，在这个范围之外为 0。这与例 3.6 求出的结果一致。

本例还提醒了我们一点：必须注意变换之后变量的取值范围。光从 (4.11) 有时并不能看清这一点。在本例中，因为 $(Y_1,Y_2)$ 是点的极坐标，其范围易于判定，在有些例子中，则需经过一定的判断。

**例 4.7** 设 $X_1,X_2$ 独立，分别服从指数分布，其中 $\lambda=1$。而设 $Y_1=X_1+X_2,Y_2=X_1-X_2$，求 $(Y_1,Y_2)$ 的密度函数。

用 (4.11) 不难算出密度函数为 $l(y_1,y_2)=\frac{1}{2}e^{-y_1}$。问题在于，这个表达式在何范围内有效？考虑到 $X_1>0,X_2>0$，推导出 $(Y_1+Y_2)/2>0,(Y_1-Y_2)/2>0$，即 $Y_1+Y_2>0,Y_1-Y_2>0$ 为变量的取值范围。

有时，我们要求的只是一个函数 $Y_1=g_1(X_1,X_2)$ 的分布。一个办法是对于任何 $y$ 找出 $\{Y_1\le y\}$ 在 $(X_1,X_2)$ 平面上对应的区域 $\{g_1(X_1,X_2)\le y\}$，记为 $A_y$。然后由 $P(Y_1\le y)=\iint_{A_y}f(x_1,x_2){\rm d}x_1{\rm d}x_2$ 找出 $Y_1$ 的分布。另一个办法是配上另一个函数 $Y_2=g_2(X_1,X_2)$，使 $(X_1,X_2)$ 到 $(Y_1,Y_2)$ 为一一对应变换，然后按照 (4.11) 找出 $(Y_1,Y_2)$ 的联合密度函数 $l(y_1,y_2)$。最后，$Y_1$ 的密度函数由公式 $\int_{-\infty}^\infty l(y_1,y_2){\rm d}y_2$ 给出。后面将给出使用这个办法的重要例子。

以上方法可完全平行地推广到 $n$ 个变量的情形：设 $(X_1,\cdots,X_n)$ 有密度函数 $f(x_1,\cdots,x_n)$，而

$$
Y_i=g_i(X_1,\cdots,X_n),i=1,\cdots,n
$$

构成 $(X_1,\cdots,X_n)$ 到 $(Y_1,\cdots,Y_n)$ 的一一对应变换，其逆变换为

$$
X_i=h_i(Y_1,\cdots,Y_n),i=1,\cdots,n
$$

此变换的 Jacobi 行列式为

$$
J(y_1,\cdots,y_n)=\begin{vmatrix}\frac{\partial h_1}{\partial y_1}&\cdots&\frac{\partial h_1}{\partial y_n}\\
\cdots&\cdots&\cdots\\
\frac{\partial h_n}{\partial y_1}&\cdots&\frac{\partial h_n}{\partial y_n}
\end{vmatrix}
$$

则 $(Y_1,\cdots,Y_n)$ 的密度函数为

$$
\begin{equation}
\tag{4.15}
l(y_1,\cdots,y_n)=f(h_1(y_1,\cdots,y_n),\cdots,h_n(y_1,\cdots,y_n))\cdot |J(y_1,\cdots,y_n)|
\end{equation}
$$
...
一个办法是考虑事件 $\{Y\le y\}=\{X_1+X_2\le y\}$，它所对应的 $(X_1,X_2)$ 坐标平面上的集合 $B$，就是下图中所示的直线 $x_1+x_2=y$ 的左下方部分。按照密度函数的定义有

![](https://s2.loli.net/2023/01/30/azHgNo3nqZj5kDf.png)

$$
P(Y\le y)=P(X_1+X_2\le y)=\iint_B f(x_1,x_2){\rm d}x_1{\rm d}x_2
$$

将重积分化为二次积分

$$
P(Y\le y)=\int_{-\infty}^\infty(\int_{-\infty}^{y-x_1}f(x_1,x_2){\rm d}x_2){\rm d}x_1
$$
...
\begin{equation}
\tag{4.16}
l(y)=\int_{-\infty}^\infty f(x_1,y-x_1){\rm d}x_1=\int_{-\infty}^\infty f(x,y-x){\rm d}x
\end{equation}
$$
...
\begin{equation}
\tag{4.17}
l(y)=\int_{-\infty}^\infty f(y-x,x){\rm d}x
\end{equation}
$$

如果 $X_1,X_2$ 独立，则 $f_(x_1,x_2)=f_1(x_1)f_2(x_2)$，这时 (4.16) 和 (4.17) 有形式

$$
\begin{equation}
\tag{4.18}
l(y)=\int_{-\infty}^\infty f_1(x)f_2(y-x){\rm d}x=\int_{-\infty}^\infty f_1(y-x)f_2(x){\rm d}x
\end{equation}
$$

这个方法在数学上有一点不足的地方是要通过在积分号下求导数，这在理论上是有条件的。另一个方法是配上另一个函数，例如 $Z=X_1$，则

$$
Y=X_1+X_2,Z=X_1
$$

构成 $(X_1,X_2)$ 到 $(Y,Z)$ 的一一对应变换，其逆变换为

$$
X_1=Z,X_2=Y-Z
$$

其 Jacobi 行列式为 $-1$，绝对值为 $1$，按照 (4.11)，得到 $(Y,Z)$ 的联合密度函数为 $f(z,y-z)$，于是 $Y$ 的密度函数 $l(y)$ 仍为 (4.16)。

**例 4.8** 设 $X_1,X_2$ 独立，分别服从正态分布 $N(\mu_1,\sigma_1^2)$ 和 $N(\mu_2,\sigma_2^2)$。求 $Y=X_1+X_2$ 的密度函数。

由 $X_1,X_2$ 独立，利用 (4.18) 的第一式，有

$$
l(y)=\frac{1}{2\pi\sigma_1\sigma_2}\int_{-\infty}^\infty\exp(-\frac{1}{2}(\frac{(x-\mu_1)^2}{\sigma_1^2}+\frac{(y-x-\mu_2)^2}{\sigma_2^2})){\rm d}x
$$

经过一些初等代数的运算，不难得到

$$
(x-\mu_1)^2/\sigma_1^2+(y-x-\mu_2)^2/\sigma_2^2=(\sigma_1^2+\sigma_2^2)^{-1}(y-\mu_1-\mu_2)^2+(ax-b)^2
$$
...
$$
\begin{align}
&a=\sqrt{\sigma_1^2+\sigma_2^2}/\sigma_1\sigma_2\\
...
$$
l(y)=\frac{1}{2\pi\sigma_1\sigma_2}\exp(-\frac{(y-\mu_1-\mu_2)^2}{2(\sigma_1^2+\sigma_2^2)})\int_{-\infty}^\infty e^{-\frac{1}{2}(ax-b)^2}{\rm d}x
$$

注意 $a,b$ 都与 $x$ 无关，作变数代换 $t=ax-b$，并利用 $\int_{-\infty}^\infty e^{-t^2/2}{\rm d}t=\sqrt{2\pi}$，即得

$$
l(y)=\frac{1}{\sqrt{2\pi(\sigma_1^2+\sigma_2^2)}}\exp(-\frac{(y-\mu_1-\mu_2)^2}{2(\sigma_1^2+\sigma_2^2)})
$$

这正是正态分布 $N(\mu_1+\mu_2,\sigma_1^2+\sigma_2^2)$ 的密度函数。由此可见，两个独立的正态变量的和仍服从正态分布，且相应的参数相加。

有趣的是，这个结论的逆命题也成立：如果 $Y$ 服从正态分布，而 $Y$ 可以表示为两个独立的随机变量 $X_1,X_2$ 之和，则 $X_1,X_2$ 必都服从正态分布。这个性质称为正态分布的**再生性**：一条蚯蚓砍成两段，每段仍成为一条蚯蚓，这是蚯蚓的再生性；同样地，一个正态变量 $Y$ 砍成两段 $X_1,X_2$（$Y=X_1+X_2$），每段仍不失其正态性。这个深刻命题的证明超出了本书的范围。

不难证明：即使 $X_1,X_2$ 不独立，只要其联合分布为二维正态 $N(\mu_1,\mu_2,\sigma_1^2,\sigma_2^2,\rho)$，则 $Y=X_1+X_2$ 仍为正态：$Y\sim N(\mu_1+\mu_2,\sigma_1^2+\sigma_2^2+2\rho\sigma_1\sigma_2)$。证明与本例类似，此处略去。

本例可以直接推广到 $n$ 个变量的情形：若 $X_1,\cdots,X_n$ 相互独立，分别服从正态分布 $N(\mu_1,\sigma_1^2),\cdots,N(\mu_n,\sigma_n^2)$，则 $X_1+X\cdots+X_n$ 服从正态分布 $N(\mu_1+\cdots+\mu_n,\sigma_1^2+\cdots+\sigma_n^2)$。
...
按本例结果有 $Z\sim N(\mu_1+\mu_2,\sigma_1^2+\sigma_2^2)$。又根据定理 3.3，知 $Z$ 与 $X_3$ 独立。再对 $Z$ 和 $X_3$ 应用本例，即得
...
* $\Gamma$ 函数：通过积分
    ...
    \begin{equation}
    \tag{4.21}
    \Gamma(x)=\int_0^\infty e^{-t}t^{x-1}{\rm d}t,x>0
    \end{equation}
    $$
...
* $\beta$ 函数：通过积分
    ...
    \begin{equation}
    \tag{4.22}
    \beta(x,y)=\int_0^1 t^{x-1}(1-t)^{y-1}{\rm d}t,x>0,y>0
    \end{equation}
    $$

    来定义，此积分在 $x>0,y>0$ 时有意义。

可以直接算出 $\Gamma(1)=\int_0^\infty e^{-t}{\rm d}t=1$，而在作变数代换 $t=u^2$ 后，算出

$$
\begin{align}
\Gamma(1/2)&=\int_0^\infty e^{-t}t^{-1/2}{\rm d}t=\int_0^\infty e^{-u^2}u^{-1}(2u{\rm d}u)\\
&=2\int_0^\infty e^{-u^2}{\rm d}u=\int_{-\infty}^\infty e^{-u^2}{\rm d}u
\end{align}
$$

令 $u=v/\sqrt{2}$，并利用 $\int_{-\infty}^\infty e^{-t^2/2}{\rm d}t=\sqrt{2\pi}$，得

$$
\Gamma(1/2)=\frac{1}{\sqrt{2}}\int_{-\infty}^\infty e^{-v^2/2}{\rm d}v=\frac{1}{\sqrt{2}}\sqrt{2\pi}=\sqrt{\pi}
$$
...
由算出的 $\Gamma(1)$ 和 $\Gamma(1/2)$，可以得到当 $n$ 为正整数时 $\Gamma(n)$ 和 $\Gamma(n+1/2)$ 的值：

$$
\begin{equation}
\tag{4.24}
\Gamma(n)=(n-1)!,\Gamma(n+1/2)=1\cdot 3\cdot 5\cdots (2m-1)2^{-m}\sqrt{\pi}
\end{equation}
$$
...
$$
k_n(x)=\begin{cases}\frac{1}{\Gamma(n/2)2^{n/2}}e^{-x/2}x^{(n/2-1)},&x>0\\
0,&x\le 0
\end{cases}
$$

是概率密度函数。实际上，由 $k_n(x)$ 的定义知道它非负，又（作变数代换 $x=2t$）

$$
\int_0^\infty e^{-x/2}x^{n/2-1}{\rm d}x=2^{n/2}\int_0^\infty e^{-t}t^{n/2-1}{\rm d}t=2^{n/2}\Gamma(n/2)
$$

故得到 $\int_{-\infty}^\infty k_n(x){\rm d}x=\int_0^\infty k_n(x){\rm d}x=1$，因而证明了其为密度函数。这个密度函数在统计学上非常重要且著名，称为**自由度 $n$ 的皮尔逊卡方密度**（相应的分布称为**卡方分布**），常记作 $\chi_n^2$。

**例 4.9** 若 $X_1,\cdots,X_n$ 相互独立，都服从标准正态分布 $N(0,1)$，则 $Y=X_1^2+\cdots+X_n^2$ 服从自由度 $n$ 的卡方分布 $\chi_n^2$。

根据例 4.5 的结果，并注意 $\Gamma(1/2)=\sqrt{\pi}$，容易验证本例的结果在 $n=1$ 时成立。于是可用数学归纳法，设此结果对 $n-1$ 成立，将 $Y$ 表示为 $Z+X_n^2$，其中 $Z=X_1^2+\cdots+X_{n-1}^2$，则由归纳假设，知 $Z$ 有密度函数 $k_{n-1}(x)$。由例 4.5 的结果知 $X_n^2$ 有密度函数 $k_1(x)$。又根据定理 3.3，知 $Z$ 与 $X_n^2$ 独立，于是按照 (4.18) 的第一式，知 $Y$ 的密度函数为

$$
l(y)=\int_{-\infty}^\infty k_{n-1}(x)k_1(y-x){\rm d}x=\int_0^y k_{n-1}(x)k_1(y-x){\rm d}x
$$

后一式是因为 $k_{n-1}(t)$ 和 $k_{1}(t)$ 都只有在 $t>0$ 时才不为 0，故有效积分区间为 $0\le x\le y$。将 (4.26) 代入上式，得

$$
l(y)=(\Gamma(\frac{n-1}{2})2^{\frac{n-1}{2}}\Gamma(\frac{1}{2})2^{\frac{1}{2}})^{-1}e^{-y/2}\cdot \int_0^yx^{\frac{n-3}{2}}(y-x)^{-\frac{1}{2}}{\rm d}x
$$
...
$$
\begin{align}
&\int_0^yx^{\frac{n-3}{2}}(y-x)^{-\frac{1}{2}}{\rm d}x\\
=&y^{\frac{n-2}{2}}\int_0^1 t^{\frac{n-3}{2}}(1-t)^{-\frac{1}{2}}{\rm d}t\\
=&y^{\frac{n-2}{2}}\beta(\frac{n-1}{2},\frac{1}{2})\\
=&y^{\frac{n-2}{2}}\Gamma(\frac{n-1}{2})\Gamma(\frac{1}{2})/\Gamma(\frac{n}{2})\\
\end{align}
$$

以此式代入 (4.27)，即得 $l(y)=k_n(y)$，从而证明了本例结果对 $n$ 也成立，从而完成了归纳证明。

本例也解释了卡方分布定义中的“自由度 $n$”。因为 $Y$ 表示为 $n$ 个独立变量 $X_1,\cdots,X_n$ 的平方和，每个变量 $X_i$ 都能任意变化，可以认为其具有一个自由度，共有 $n$ 个变量，因此有 $n$ 个自由度。当然这个解释只在 $n$ 为正整数时才有效（注意 $k_n(x)$ 的定义当中并不必限制 $n$ 为正整数，只需要 $n>0$）；实际上，自由度这个名词通常也只在 $n$ 为正整数时被使用。

卡方分布具有下列重要性质：

1. 设 $X_1,X_2$ 独立，$X_1\sim\chi_m^2,X_2\sim\chi_n^2$，则 $X_1+X_2\sim\chi_{m+n}^2$

    证明可以直接利用和的密度公式 (4.18) 得到。更简单的方法是从卡方变量的表达式出发，设 $Y_1,\cdots,Y_{m+n}$ 独立且都服从分布 $N(0,1)$。令 $X_1=Y_1^2+\cdots+Y_m^2,X_2=Y_{m+1}^2+\cdots+Y_{m+n}^2$，按照本例有

    $$
    X_1\sim\chi_m^2,X_2\sim\chi_n^2
    $$

    而 $X_1+X_2=Y_1^2+\cdots+Y_{m+n}^2$，按照本例有
...
2. 若 $X_1,\cdots,X_n$ 独立，且都服从指数分布，则
    ...
    首先，由 $X_i$ 的密度函数为 $\lambda e^{-\lambda x},x>0$，知 $2\lambda X_i$ 的密度函数为 $\frac{1}{2}e^{-x/2},x>0$。但在 (4.26) 中令 $n=2$，可知这正好是 $\chi_2^2$ 的密度函数，因此 $2\lambda X_i\sim\chi_2^2$。再因 $X_1,\cdots,X_n$ 独立，利用性质 1，即得证。

### 随机变量商的密度函数

设 $(X_1,X_2)$ 有密度函数 $f(x_1,x_2)$，$Y=X_2/X_1$，要求 $Y$ 的密度函数。为简单起见，限制 $X_1$ 只取正值。

事件 $\{Y\le y\}=\{X_2/X_1\le y\}$ 等价于 $\{X_2\le X_1y\}$，因为 $X_1>0$。这相应于下图中标出的区域 B。通过化重积分为二次积分，得到

![](https://s2.loli.net/2023/02/01/p9bCxlo1K2TkiIm.png)

$$
P(Y\le y)=\iint_Bf(x_1,x_2){\rm d}x_1{\rm d}x_2=\int_0^\infty(\int_{-\infty}^{x_1y}f(x_1,x_2){\rm d}x_2){\rm d}x_1
$$
...
\begin{equation}
\tag{4.28}
l(y)=\int_0^\infty x_1f(x_1,x_1y){\rm d}x_1
\end{equation}
$$

若 $X_1,X_2$ 独立，则 $f(x_1,x_2)=f_1(x_1)\cdot f_2(x_2)$，而上式变为

$$
\begin{equation}
\tag{4.29}
l(y)=\int_0^\infty x_1f_1(x_1)f_2(x_1y){\rm d}x_1
\end{equation}
$$

(4.28) 式也可以通过增加一个变量 $Z=X_1$，再利用 (4.11) 和 (2.9) 得到。这个做法不需要在积分号下求导数。

下面考察两个在统计学上非常重要的例子。

**例 4.10** 设 $X_1,X_2$ 独立，$X_1\sim\chi_n^2,X_2\sim N(0,1)$，而 $Y=X_2/\sqrt{X_1/n}$，求 $Y$ 的密度函数。

记 $Z=\sqrt{X_1/n}$，先求 $Z$ 的密度函数 $g(z)$，有

$$
P(Z\le z)=P(\sqrt{X_1/n}\le z)=P(X_1\le nz^2)=\int_0^{nz^2}k_n(x){\rm d}x
$$

两边对 $Z$ 求导，得到 $Z$ 的密度函数为
