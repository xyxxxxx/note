...
有时候，多个列表会引用同一个列表对象，这一点常常会令Python初学者感到困惑，例如：
...
这里重复了3次的列表`[1]`实际上引用的是同一列表，因此修改任意一个列表，所有的列表都会被同样地修改。要想创建多维列表，推荐使用列表推导式。再例如：
...
拷贝列表时要根据实际需求确定，是引用同一列表还是创建独立的副本，下面演示了三种拷贝列表的方法：
