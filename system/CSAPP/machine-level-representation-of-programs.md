...
理解跳转指令的目标如何编码对于研究链接非常重要，并且也能帮助理解反汇编器的输出。在汇编代码中，跳转目标用符号标号表示，汇编器和链接器会产生跳转目标的适当编码。跳转指令有几种不同的编码，最常用的是**PC相对的（PC-relative）**，也就是将目标指令的地址与跳转指令的下一条指令的地址之间的差作为编码，这样的地址偏移量可以编码为1, 2或4个字节；另一种编码方法是给出”绝对“地址，用4个字节直接指定目标，汇编器和链接器会选择适当的跳转目的编码。
