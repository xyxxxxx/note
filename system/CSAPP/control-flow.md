...
内核为每个进程维持一个**上下文（context）**。上下文就是<u>内核重新启动一个被抢占的进程所需的状态</u>。它由一些对象的值组成，包括<u>通用目的寄存器、浮点寄存器、程序计数器、用户栈、状态寄存器、内核栈和各种内核数据结构</u>，比如描述地址空间的页表、包含当前进程信息的进程表，以及包含进程已打开文件的信息的文件表。
