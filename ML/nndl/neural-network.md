...
假设一个神经元接收$$D$$个输入$$x_1,x_2,\cdots,x_D$$，令向量$$\pmb x=[x_1,x_2,\cdots,x_D]$$来表示这组输入，并用**净输入(net input)**$$z\in \R$$表示一个神经元获得的输入信号$$\pmb x$$的加权和，
$$
z=\sum_{d=1}^Dw_dx_d+b\\
=\pmb w^{\rm T}\pmb x+b
$$
其中$$\pmb w=[w_1,w_2,\cdots,w_D]\in \R^D$$是$$D$$维的权重向量，$$b\in\R$$是偏置。
...
![Screenshot from 2020-09-15 10-28-33.png](https://i.loli.net/2020/09/15/YMHfE6OiSJTqrZy.png)
...
![Screenshot from 2020-09-15 11-10-23.png](https://i.loli.net/2020/09/15/A9GWxr7Tov1lyqQ.png)
...
每个 Maxout 单元有$$K$$个权重向量$$\pmb w_k\in \R^D$$和偏置$$b_k(1\le k\le K)$$。对于输入$$\pmb x$$，可以得到$$K$$个净输入$$z_k=\pmb w_k^{\rm T}\pmb x+b_k,1\le k\le K$$。Maxout 单元的非线性函数定义为
$$
...
通过一定的连接方式或信息传递方式进行协作的神经元可以看作一个网络，即神经网络。
